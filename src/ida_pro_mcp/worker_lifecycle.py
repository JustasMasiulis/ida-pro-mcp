...
class WorkerLifecycle:
    ...
    def __init__(
        ...
        *,
        ...
        self.idle_ttl_sec = idle_ttl_sec if idle_ttl_sec is not None else self.IDLE_TTL_SEC
    ...
    def check_shutdown_reason(self) -> str | None:
        """Return a non-empty string if the watchdog should fire shutdown,
        or None to keep running."""
        with self._lock:
            last_req = self._last_request_at
        now = time.monotonic()
        if (now - last_req) > self.idle_ttl_sec:
