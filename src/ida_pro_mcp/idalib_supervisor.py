...
class IdalibSupervisor:
    ...
    def _find_instance_for_path(
        ...
        for instance in instances:
            if backend is not None and instance.get("backend", "gui") != backend:
                continue
            ...
            unadopted.append(
                {
                    ...
                    "last_accessed": "",
                    "is_analyzing": False,
                    "metadata": {"backend": "gui", "host": inst.get("host"), "port": inst.get("port")},
                    "is_active": reachable,
                    "backend": "gui",
                    "owned": False,
                    "adopted": False,
...
def idb_list() -> IdalibListResult:
    """List open database workers and running GUI IDA instances (adopted=False entries are GUI processes not yet opened through idb_open)."""
    sup = _require_supervisor()
    try:
...
def databases_resource() -> dict:
    ...
    sup = _require_supervisor()
    return {"databases": sup.list_sessions(), "count": len(sup.sessions)}
